...
import com.bytatech.ayoos.consultation.config.KafkaProperties;
...
class ConsultationKafkaResourceIT {
    ...
    void setup() {
        ...
        ConsultationKafkaResource kafkaResource = new ConsultationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/consultation-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/consultation-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
